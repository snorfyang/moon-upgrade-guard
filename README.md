...
是有意暂不支持的，见[已知限制](#已知限制)；后续计划见[路线图](ROADMAP.md)。
...
存储 gap 使用 `__gap` 定长数组约定：gap 覆盖的字节是未使用的，因此后续版本可以把
它们用于新变量，也可以整体替换该 gap，前提是取代它的内容结束在 gap 原先结束的
那个字节——这正是让声明在它之后的变量保持在原位的原因。仅凭名字永远不会跳过比较：类型必须是定长
数组（动态数组的 slot 存放长度而非保留字节），元素类型必须保持不变，而结束位置发生变化的 gap 仍会被报告为移动。
